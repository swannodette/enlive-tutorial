...
The final tutorial is fairly advanced. It illustrates how Clojure, being a Lisp, can in fact template itself! I use this ability to illustrate how a downside to Enlive can be overcome. For example, you'll often have a widget whose elements you might just want reply by CSS id (or class). Enlive syntax makes this somehwat tedious for the simple case. With macros you can achieve the same level of simplicity even though it is not a feature that ships with Enlive!
