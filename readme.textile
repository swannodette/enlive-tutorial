...
Open the file *template2.html* in your favorite web browser. It just page with a list of links, not that special. Point your browser at *http://localhost:8080/*. You should see pretty much the same thing except that we've dynamically inserted links.

Wow, how did we do that if we have no inline code to define the loop? Let's get into the code. Open up *template2.clj* in your favorite text editor. At the top of the file you should see the by now familiar namespace declaration. After that we declare a variable for holding a dummy context which we're going to pass to our template.

<pre>
(def dummy-context
     {:title "Enlive Template2 Tutorial"
      :links [{:text "Clojure" :href "http://www.clojure.org"}
              {:text "Compojure" :href "http://www.compojure.org"}
              {:text "Clojars" :href "http://www.clojars.org"}
              {:text "Enlive" :href "http://github.com/cgrand/enlive"}]})
</pre>

Clojure makes it easy to define nested data structures. *dummy-context* is just a hash-map (aka dictionary, aka associative array) that two key-value pairs. The first pair is for the title of the page. The second pair is for the list of links. Here we have a Clojure vector of hash-maps. If you're used to building up JSON data structures this shouldn't look too foreign to you.

Following that we encounter something new, *html/defsnipped* (don't for get that *html* is just an alias for *net.cgrand.enlive-html*):
...
Snippets are a ver cool feature of Enlive. They allow you to grab any peice of HTML out of a larger HTML file and make a kind of mini-template out of it. In this case we're getting the first-child of the element that matches the CSS selector *ul#links*. This snippet destructures it's first argument (a hash-map) to extract the value of the for the key *:text* and the key *:href*. We're also introduced to *html/do->*. This is a convenience, we often want to take the matched element and apply a series of transformations on it. In this case we want to set the content of the node as well as it's href attribute.

No let's at the template to see how we put this together:

<pre>
(html/deftemplate index "tutorial/template2.html"
  [{title :title links :links}]
  [:#title]   (html/content title)
  [:ul#links] (html/content (map link-model links)))
</pre>

The first match pair of the template should be familiar to you by now. The second pair is the foor loop. While the previous snippet may have seemed a bit verbose, this lines shows just how powerful Enlive is. Once you defined some snippets and templates you can really compose them which ever way you like and see fit.
