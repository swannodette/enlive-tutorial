...
We want to be able handle the inner loop. On one level, as you'll see, there's a little bit less typing involved in the traditional approach. However you will have something that's a lot less reusable. That because the the inner link loop and the outer section loop are hopelessly interwinted. You may have many pages on your site that use the same section pattern but not the internal link pattern. But since these can't be separated you'll have to do some copy and paste. Not so with Enlive. Enlive makes you do a little more work upfront and very, very little work later on.
...
This is very important. We only want to match only the first ul element that we find that has the links class and only the very child inside that. This is the selector that gets the job done. It's analagous to:

<pre>
ul.links:nth-of-type(1) > *:first-child
</pre>

It's important to note that using *nth-of-type* requires an extra pair of brackets around the element *ul*. Take note of this. This extra pair of brackets is easy to forget. Whenever you want to be more specfic about what type of tag you want to match (beyond matching on CSS id or class) you'll need an extra pair of brackets.
...
For example say on a different page you only want the list of links. Instead of creating a whole new template file and repeating yourself with copy and paste you can do the following:
...
That was easy and DRY.

Most template solutions are a giant mess of uncomposable for loops. It's true that for trivial cases Enlive is a bit more work, but for anything remotely like a real website, Enlive has real advantages. *template2.html* can actually show what the page will look like with dummy extra list items, that is the designer doesn't need to start webserver or install any web framework at all, he/she can work directly in HTML and CSS. And you can mix and match the various parts of the designed HTML as you see fit.
