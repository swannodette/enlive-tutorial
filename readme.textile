...
If somehow you happen to stumble upon this tutorial, please note that it is not even close to being complete. Proceed at your own peril.

h1. An Introduction to Enlive

Though Enlive has been around for sometime now the Clojure community has been slow to embrace this useful library. I believe this is due simply to the lack of good introductory documentation. This is no slam against Christophe Grand. He's a busy man and I'd rather he write tutorials on optimizing Clojure than create introductory tutorials about Enlive. In anycase contribution is what open source communities are all about, right?
...
This is a bit easier on the eyes. But as many a coder has discovered this isn't very composable and you're stuck with a limited subset of your programming language.

This has driven many programmers to eschew templating DSLs and to generate markup directly in code. While this is fast and flexible since you now have the power of function composition it means that you're putting quite a bit of distance between yourself and a designer who is very comfortable with HTML/CSS.

So can we have our cake and eat it too? There are a few existing solutions that approach the idea from an interesting "direction":http://beebole.com/pure/.

Which brings us to Enlive. Enlive gives you the advantages of designer accessibe templates (since they're just HTML) without not losing the power of fuction composition.
...
** a) What selectors match the part of the HTML document that I care about?
** b) What templates and snippets do I need write?
...
On one level, as you'll see, there's a lot less typing involved in this. However you all have something that's a lot less reusable. The link loop and section loop are hopelessly interwinted. You may have many pages on your site that use the same section pattern but not the internal link pattern. But since these can't be separated you'll have to do some copy and paste. Not so with Enlive. Enlive makes you do a little more work upfront and very, very little work later on.
...
This is very important. We only want to match only the first ul element that we find that has the links class and only the very child inside that. This is the selector that gets the job done. The snippet that will template this component of your page look like the following:

Now that we have our selector *defsnippet* (don't forget that *html* is just an alias for *net.cgrand.enlive-html*):
...
Snippets are like templates with two main differences. First, snippets take a selector. This means that they can match only specific parts of an HTML document. The function produced by a *defsnippet* returns transformed content, <i>not</i> a list of strings the *deftemplate* does. This snippet destructures it's first argument (a hash-map) to extract the value of the keys *:text* and *:href*. We're also introduced to *html/do->*. This is a convenience, we often want to take the matched element and apply a series of transformations on it. In this case we want to set the content of the node as well as it's href attribute.
...
Here we have eto use emit* because snippets return a sequenes of nodes not strings the way templates do.

Okay now we want to loop over the sections. A section is a *h2* tag followed by a *ul* tag. Again we need to figure out the correct selector. This time we're trying to emulate the following popular pattern:
...
Note that unlike the previous example what we're looping over has no "container". That is, there is no surrouding element for the adjacent *h2* and *ul* tags. Enlive recently added support for "ranges" making it simple to also express this pattern with Enlive templates.
...
For example say on a different page you only want the list of links. Instead make a whole new template and repeating yourself with copy and paste you can do the following:
...
Most templates are a giant mess of uncomposable for loops. For trivail cases Enlive is a bit more work, but for anything remotely like a real website, Enlive has some real advantages. *template2.html* can actually show what the page will look like with dummy extra list items. The designer doesn't need to start webserver or install any web framework at all, he/she can work directly with HTML. And you can mix match the various parts of the design as you see fit.
