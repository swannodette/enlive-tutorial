...
There are two real camps in HTML templating. The first which almost everyone is familiar is the PHP style template:
...
This is a bit easier on the eyes. But as many a coder has discovered this just isn't that composable and you're stuck with a pathetic subset of your programming language.

This has driven many programmers to eschew templating DSLs and to just generate markup in code. While this is fast and flexible since you now have the power of function composition it also means that you're putting quite a bit of distance between yourself and designer who is very comfortable with HTML/CSS.

Which brings us to Enlive. Enlive give you all the advantages of a designer accessibility of a HTML templating DSL while not losing the power of composition you get from generating markup in code.L Enlive is all about having your cake and eating it too.
...
Again we need to figure out the correct selector. This time we're trying to emulate the following popular pattern:
...
Not that unlike the previous example what we're looping over has no "container". Enlive recently added support for "ranges" making it simple to also express this pattern with Enlive templates.

Our HTML has some dummy content again. We only care about the first unique pair of *h2* and *p* tags.
...
Most templates are a giant mess of unreadable for loops. While it does seems like a little work to achieve the same effect with Enlive, consider the following. *template2.html* can actually show what the list will look like with dummy extra list items. These are removed because we only match the first child with our snippet. The designer doesn't need to start webserver or install any web framework at all, he can work directly with HTML.
